...
class EMAIL {
// TODO: Stop header injection at sender name and body
// TODO: Add advance attachment control (Remote attachment, Remote file size)
// TODO: Set Global error string array.
// TODO: Modularize
// TODO: Size of the file
// TODO: Limit total attachment size.
...
//function to get list of engineers
	function getEngineerList($platform) {
		global $mysqlserver;
...
//Implementors
		d($platform);
		//$engineer_data = "SELECT * FROM NOCTOOLS.employees where status = 'e';";
			$engineer_data = "select b.emailAddress from employees b join MopPlatforms a on a.engineer = b.Id where platform = '$platform[0]';";
	    $engineer_data_result=$dbh->query($engineer_data) or die ("Query to get gsx data from sonus_gsx failed: ".mysql_error());

    	foreach ($engineer_data_result as $engineer ) {
			$engineerList.="$engineer[emailAddress];";
		}

		//$engineer_data = "SELECT smsaddress FROM NOCTOOLS.employees where smsaddress is not NULL;";
			$engineer_data = "select b.smsaddress from employees b join MopPlatforms a on a.engineer = b.Id where platform = '$platform[0]';";
	    $engineer_data_result=$dbh->query($engineer_data) or die ("Query to get gsx data from sonus_gsx failed: ".mysql_error());

    	foreach ($engineer_data_result as $engineer ) {
			$engineerList.="$engineer[smsaddress];";
		}

		$engineerEmailList = substr($engineerList,0, strlen($engineerList)-1);
		d($engineerEmailList);
		return($engineerEmailList);
	}
...
$to[] = "rahul.pache@gmail.com";
//$cc[] = "rahul_200611079@yahoo.com";
//$mail->addTo("rahul.pache@gmail.com");
//$mail->addTo("rahul_200611079@yahoo.com");
...
eol();
//echo $mail->addReplyTo("reply@rahulprasad.com");
//eol();
echo $mail->addSubject("A Hullo");
eol();
